  ...
  const options = [
    ...
    },
    {
      ...
      provider: (options?: Partial<MulticallOptions>) =>
        new Web3Provider(new JsonRpcRouter([multicallMiddleware(options)], new JsonRpcExternalProvider(ganache.spyProxy)))
    },
    {
      name: 'Ether.js external provider wrapper',
      provider: (conf?: Partial<MulticallOptions>) =>
        new Web3Provider(new MulticallExternalProvider(new JsonRpcExternalProvider(ganache.spyProxy), conf))
    },
    {
      ...
      provider: (conf?: Partial<MulticallOptions>) => {
        ...
        engine.push(
          providerAsMiddleware(new MulticallExternalProvider(new JsonRpcExternalProvider(ganache.spyProxy), conf) as any)
        )

        return new ethers.providers.Web3Provider(providerFromEngine(engine) as any)
      }
    },
    {
      ...
      provider: (conf?: Partial<MulticallOptions>) => {
        ...
        return new Web3Provider(new MulticallExternalProvider(spyHttpProvider as any, conf))
      }
    },
    {
    ...
    },
    {
      ...
      provider: (options?: Partial<MulticallOptions>) =>
        new Web3Provider(new JsonRpcRouter([multicallMiddleware(options)], new JsonRpcExternalProvider(ganache.provider))),
      ignoreCount: true
    },
    {
      name: 'Ether.js external provider wrapper (without proxy)',
      provider: (conf?: Partial<MulticallOptions>) =>
        new Web3Provider(new MulticallExternalProvider(new JsonRpcExternalProvider(ganache.provider), conf)),
      ignoreCount: true
    },
    {
      ...
      provider: (conf?: Partial<MulticallOptions>) => {
        ...
        engine.push(
          providerAsMiddleware(new MulticallExternalProvider(new JsonRpcExternalProvider(ganache.provider), conf) as any)
        )
    ...
    },
    {
      ...
      provider: (conf?: Partial<MulticallOptions>) => {
        ...
        return new Web3Provider(new MulticallExternalProvider(web3HttpProvider as any, conf))
      },
      ignoreCount: true
    }
  ]
    ...
    context(option.name, () => {
      ...
      describe('Handle errors', async () => {
        it('Should not retry after failing to execute single call (not multicalled)', async () => {
          ...
          await expect(multiCallMockB.callStatic.testCall(1, '0x1122')).to.be.rejectedWith(
